-- ----------------------------------------------------------------------------
-- BigQuery DDL for nbcs_ods (30 tables)
-- Source: hive/ddl/05-ods-cleanse.hql     (15 tables)
--         hive/ddl/06-ods-delta-scd2.hql  (8 delta-merged + 3 SCD-2 = 11)
--         hive/ddl/07-ods-acid.hql        (4 ACID transactional tables)
--
...
--   15 ODS cleanse:     PARTITION BY snapshot_date/event_date/call_date/sched_date (DATE)
--    8 ODS delta-merged: PARTITION BY work_month/period_month/event_date/swap_month/
--                        event_month/snapshot_date (DATE)
--    3 SCD-2:           RANGE_BUCKET(eff_from_year, GENERATE_ARRAY(2015,2030,1))
--    4 ACID:            No partition; CLUSTER BY primary key
--                        (transactional=true, CLUSTERED BY…INTO N BUCKETS dropped)
-- ----------------------------------------------------------------------------

-- ============================================================================
-- 05: Cleansed / conformed entities (15 tables)
-- ============================================================================
...
-- ============================================================================
-- 06: Delta-merged entities (8 tables)
-- ============================================================================
...
-- ============================================================================
-- 06 (cont): SCD-2 history tables (3 tables)
-- Partition by RANGE_BUCKET on eff_from_year (INT64)
-- ============================================================================
...
PARTITION BY RANGE_BUCKET(eff_from_year, GENERATE_ARRAY(2015, 2030, 1));
...
-- ============================================================================
-- 07: Former Hive ACID transactional tables (4 tables)
-- transactional='true' dropped (BigQuery natively supports DML on all tables).
-- CLUSTERED BY … INTO N BUCKETS → CLUSTER BY (primary key); bucket count dropped.
-- STORED AS ORC → standard BigQuery columnar storage.
-- ============================================================================
