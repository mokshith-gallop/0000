-- ----------------------------------------------------------------------------
-- BigQuery DDL for nbcs_dm (25 tables: 9 dimensions, 9 facts, 7 aggregates)
-- Source: hive/ddl/08-dm-tables.hql
-- NOTE: The 15 dm views (09-dm-views.hql) are out-of-scope — authored by the
--       Transform flow.
--
...
--    9 dimensions:   No partition; CLUSTER BY surrogate key
--    9 facts:        RANGE_BUCKET on date_key INT64 (8 tables) or
--                    PARTITION BY period_month DATE (fact_billing_line)
--    7 aggregates:   RANGE_BUCKET on date_key/week_start_key INT64 (4 tables) or
--                    PARTITION BY period_month DATE (3 tables)
--
-- fact_interaction: multi-column Hive partition (date_key, channel) collapsed →
--   PARTITION BY RANGE_BUCKET(date_key, ...) + CLUSTER BY channel, agent_sk
--   OPTIONS(require_partition_filter = true)
-- ----------------------------------------------------------------------------

-- ============================================================================
-- Dimensions (9 tables) — no partition, CLUSTER BY surrogate key
-- ============================================================================
...
-- Multi-column Hive partition (date_key INT, channel STRING) collapsed:
-- date_key → RANGE_BUCKET partition
-- channel  → CLUSTER BY (+ agent_sk from legacy CLUSTERED BY … INTO 16 BUCKETS)
...
-- period_month promoted from STRING to DATE; CLUSTER BY client_sk, program_sk
...
PARTITION BY RANGE_BUCKET(week_start_key, GENERATE_ARRAY(20200101, 20301231, 100));

-- period_month promoted from STRING to DATE
