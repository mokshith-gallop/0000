...
-- Source: hive/ddl/02-staging-sqoop-mirrors.hql (27 tables)
--         hive/ddl/03-staging-delta-feeds.hql   (8  tables)
--         hive/ddl/04-staging-file-feeds.hql     (10 tables)
--
-- Type mapping per locked Schema and Type Mapping decision:
--   STRING→STRING, BIGINT→INT64, INT→INT64, BOOLEAN→BOOL,
--   DOUBLE→FLOAT64, DECIMAL(p,s)→NUMERIC(p,s), TIMESTAMP→TIMESTAMP
--   MAP<K,V>→ARRAY<STRUCT<key K, value V>>, ARRAY<T>→ARRAY<T>
--
-- Partitioning per locked Performance Optimization decision:
--   27 sqoop mirrors:  PARTITION BY load_date (DATE)
--    8 delta feeds:    PARTITION BY extract_ts (DATE)
--   10 file feeds:     PARTITION BY feed_date (DATE), CLUSTER BY client_code
--
-- Epoch column COMMENTs carried as OPTIONS(description='...').
-- Lie columns (issued_ts_sec, due_ts_sec) carry explicit warnings per
-- docs/EPOCH-POLICY.md.
-- ----------------------------------------------------------------------------

-- ============================================================================
-- 02: Sqoop-landed RDBMS mirrors (27 tables)
-- ============================================================================
...
-- Multi-column Hive partition (load_date, site_code) collapsed:
-- load_date → PARTITION BY, site_code → CLUSTER BY
...
-- Legacy Hive CLUSTERED BY (call_id) INTO 16 BUCKETS → CLUSTER BY call_id
...
-- Lie columns: issued_ts_sec and due_ts_sec — names say "seconds" but values
-- are MILLISECONDS.  See docs/EPOCH-POLICY.md.
...
-- ============================================================================
-- 04: SFTP / file-landed client feeds (10 tables)
-- Multi-column Hive partition (client_code, feed_date) collapsed:
--   feed_date → PARTITION BY (DATE), client_code → CLUSTER BY (STRING)
-- ============================================================================
...
-- Complex type: sections ARRAY<STRUCT<...>> — Hive INT widened to INT64 inside STRUCT
...
-- Complex types: messages ARRAY<STRUCT<...>>, metadata MAP→ARRAY<STRUCT<key,value>>
...
-- Complex type: keywords ARRAY<STRING> (direct mapping)
