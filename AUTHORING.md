...
### Cross-dataset views — multi-dataset builds (`dataset_map`)

When the schema has **views that read tables in *other* datasets** (e.g. a `dm` view does
`FROM ods.org_unit`), one build dataset isn't enough — the qualified `ods.` ref won't resolve in
`dmt_build`. Build **one dataset per layer** and declare a `dataset_map`:

```yaml
migration:
  dataset_map:                  # real dataset -> build dataset; remaps qualified cross-dataset refs
    staging: dmt_build_staging
    ods:     dmt_build_ods
    dm:      dmt_build_dm
  steps:
    - { kind: ddl, dataset: dmt_build_ods, sql: ddl/03-ods.sql }       # ods tables  -> dmt_build_ods
    - { kind: ddl, dataset: dmt_build_dm,  sql: ddl/05-dm-views.sql }  # dm views; FROM ods.x -> dmt_build_ods.x
suites:
  - { pattern: schema_conformance, target_dataset: dmt_build_dm,
      tables: [ { table: vw_org, expect_object_type: VIEW, columns: [ ... ] } ] }
```

- A step's `dataset:` is the build dataset it lands in. It **MUST** be `dmt_build` or `dmt_build_*` —
  the harness refuses to create/reset/drop a real/named dataset (it must never touch a real target),
  so map `ods` → `dmt_build_ods`; never build into `ods` directly.
- `dataset_map` remaps the views' production refs (`ods.x` → `dmt_build_ods.x`) **at build time only** —
  your committed DDL keeps its real `ods.` refs, and you still validate the exact view.
- The harness provisions every build dataset, applies the steps, verifies, and **tears them all
  down** (create → verify → destroy); nothing is left in BigQuery.
- Validate built objects against their build dataset (`target_dataset: dmt_build_dm`).
- The map is generic — any datasets, any names (`raw`/`clean`/`marts`, etc.); the harness only ever
  owns `dmt_build*`.
