...
def run_mvs(data: dict, base_dir: str = ".") -> Report:
    ...
    build_datasets = None
    if migration:
        build_datasets = _build(migration, ctx, report, base_dir)
        if build_datasets is None:        # build failed -> report already carries the ERROR suite
            return report
    ...
    try:
        ...
        from . import build
        if build_datasets and _transient_build(migration):
            for ds in build_datasets:   # create -> verify -> tear down: leave BigQuery as it was
                build.teardown(ctx.target, ds)
        if source_created:
...
def _transient_build(migration: dict) -> bool:
    """Build datasets are transient — torn down after the run — when `isolate` (unique per-run
    names) or multi-dataset (steps name real datasets that must not linger). A single shared
    `dmt_build` (neither) persists: reset-at-start, reusable/inspectable between runs."""
    return bool(migration.get("isolate") or any(s.get("dataset") for s in migration.get("steps", [])))


def _build(migration: dict, ctx: Context, report: Report, base_dir: str) -> list[str] | None:
    """Provision the build dataset(s) and apply the CUT's steps verbatim. A step may target its
    own `dataset` (multi-dataset builds, so cross-dataset views like `ods.x` resolve); steps
    without one land in the default build dataset. Returns the list of provisioned datasets (for
    teardown), or None if the build failed (report gets an ERROR suite)."""
    from . import build
    ...
    if ctx.target_kind != "bigquery":
        ...
        return None
    bq = ctx.target
    default_name = migration.get("build_dataset") or build.BUILD_DS_DEFAULT
    dataset_map = migration.get("dataset_map") or {}
    steps = migration.get("steps", [])
    # distinct datasets to provision: each step's `dataset`, or the default for steps without one
    datasets = list(dict.fromkeys((s.get("dataset") or default_name) for s in steps)) or [default_name]
    provisioned: list[str] = []
    try:
        for ds in datasets:
            build.provision_build_dataset(bq, ds)
            provisioned.append(ds)
        for step in steps:
            build.apply_step(bq, default_name, step, base_dir, dataset_map)
        return provisioned
    except Exception as e:  # noqa: BLE001 — any build failure -> ERROR (don't judge a half-built target)
        if _transient_build(migration):
            for ds in provisioned:   # never leave a transient (real-named / isolated) dataset behind
                try:
                    build.teardown(bq, ds)
